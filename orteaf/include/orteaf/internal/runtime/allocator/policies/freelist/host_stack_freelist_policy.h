...
public:
    ...
    void initialize(Resource* resource) {
        ORTEAF_THROW_IF_NULL(resource, "HostStackFreelistPolicy requires non-null Resource*");
        resource_ = resource;
    }
    ...
    std::size_t get_active_freelist_count() const { return stacks_.empty() ? 0 : 1; }
    ...
    void expand(std::size_t list_index, const MemoryBlock& chunk, std::size_t chunk_size, std::size_t block_size) {
        ...
        for (std::size_t i = 0; i < num_blocks; ++i) {
            ...
            MemoryBlock block{chunk.id, Resource::makeView(chunk.view, offset, block_size)};
            stacks_[list_index].pushBack(std::move(block));
        }
    }
